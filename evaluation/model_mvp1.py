...
def preprocess_df(df):
    ...
    # only core
    df["lan_split"] = df["predict_lan"].apply(lambda x: "core" if x not in NON_CORE_LANG else "core")
...
def recsys(query, limit, model):
    ...
    lang = "core" if lang not in NON_CORE_LANG else "core"
...
def recsys_batch(tags: List[str], limit=5, model=None):
    ...
    df_eval["lan_split"] = df_eval["predict_lan"].apply(lambda x: "core" if x not in NON_CORE_LANG else "core")
...
if __name__ == "__main__":
    ...
    output_stack = pd.concat([output['core']])  # output is split_tags_y, join on tags_eval
    ...
    fig = ax.get_figure()
    fig.savefig('evaluation/mvp1.png')